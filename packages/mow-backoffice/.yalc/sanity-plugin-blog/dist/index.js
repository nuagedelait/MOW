...
const baseLang = "fr";
var settings$1 = (S, additionnals) => S.list().id("build-level").title("Build").items([S.listItem().title("Settings").icon(ControlsIcon).child(S.document().schemaType("siteSettings").documentId("siteSettings")), S.listItem().title("Menus").id("menu-docs").icon(MenuIcon).schemaType("menu").child(S.documentList().id("menus").title("Menus").filter('_type == "menu" && (!defined(_lang) || _lang == $baseLang)').params({
  baseLang
})), S.listItem().title("Categories").id("cat-docs").icon(FolderIcon).schemaType("category").child(S.documentList().id("category").title("Category").filter('_type == "category" && (!defined(_lang) || _lang == $baseLang)').params({
  baseLang
})), S.listItem().title("Forms").id("forms-docs").icon(DocumentTextIcon).schemaType("form").child(S.documentList().id("form").title("Forms").filter('_type == "form" && (!defined(_lang) || _lang == $baseLang)').params({
  baseLang
})), additionnals(S)]);
var addresses = S => {
  ...
  if (hasAddress) {
    return [S.listItem().title("Addresses").id("address-docs").icon(PinIcon).schemaType("address").child(S.documentList().id("address").title("Address").filter('_type == "address" && (!defined(_lang) || _lang == $baseLang)').params({
      baseLang
    })), S.listItem().title("Maps").id("maps-docs").icon(PinIcon).schemaType("map").child(S.documentList().id("maps").title("Maps").filter('_type == "map" && (!defined(_lang) || _lang == $baseLang)').params({
      baseLang
    }))];
  } else {
    return [];
...
var documents$1 = (S, additionnals) => S.list().id("doc-level").title("Documents").items([S.listItem().title("Posts").id("post-docs").icon(DocumentIcon).schemaType("post").child(S.documentList().id("post").title("Posts").filter('_type == "post" && (!defined(_lang) || _lang == $baseLang)').params({
  baseLang
})), S.listItem().title("Pages").id("page-docs").icon(ComposeIcon).schemaType("page").child(S.documentList().id("page").title("Pages").filter('_type == "page" && (!defined(_lang) || _lang == $baseLang)').params({
  baseLang
})), ...addresses(S)]);
var structure = additionnals => {
  return S => S.list().title("Content").items([S.listItem().title("Build").icon(ControlsIcon).child(settings$1(S, additionnals.settings)), S.listItem().title("Documents").icon(DocumentIcon).child(documents$1(S))]);
};
var page = {
  title: "Page",
  name: "page",
...
var categories = {
  ...
  preview: {
    ...
    prepare(preparePros) {
      ...
      return {
        title,
        media: "<div class=\"sanity-studio__preview-fallback-icon\">".concat(icon.icon, "</div>")
      };
    }
  }
};
...
var documents = [categories, page, post, settings, menu, gallery];
var openGraph = {
  ...
  preview: {
    ...
    prepare(_ref) {
      ...
      return {
        ...
        subtitle: "".concat(width, "x").concat(height)
      };
    }
  }
};
...
var objects = [openGraph, pagebuilder, captionImage, size, ...nav, tab];
